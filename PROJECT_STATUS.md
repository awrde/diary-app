...
- **대시보드 로딩 최적화**: 최근 일기 목록을 최대 14개로 경량 로드, DiaryCard 프리뷰 길이 축소로 렌더 부담 감소.
- **차트 렌더 지연 완화**: MetricChart 애니메이션/응답성 애니메이션 비활성화로 초기 드로잉 지연 제거.
- **모바일 레이아웃 안정화**: 모바일에서 카드 블러/트랜지션 제거, 주요 섹션 `min-height` 추가로 초기 깜빡임과 겹침 완화.
